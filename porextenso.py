...
Reserved = ["imprimir", "ler", "enquanto", "se_e_somente_se", "casocontrario", "falso", "verdadeiro", "booleano" ,"inteiro", "string", "retornar"]
...
class IfNode(Node): 
    def Evaluate(self, symbtable):
        if (self.children[0].Evaluate(symbtable)[1] == "string"):
            raise ValueError("ERRO: se_e_somente_se nao pode ser com somente uma string")
    ...
    def selectNext(self):
        ...
        if(self.position < len(self.origin)):
            ...
            if self.origin[self.position].isalpha():
                ...
                # Reserved = ["imprimir", "ler", "enquanto", "se_e_somente_se", "casocontrario", "false", "verdadeiro", "booleano" ,"inteiro", "string", "retornar"]
                ...
                # Palavras reservadas
                    ...
                    self.actual = Token("println", "println")
                elif(text == "se_e_somente_se"):
                    self.actual = Token("if", "if")
                elif(text == "enquanto"):
                    self.actual = Token("while", "while")
                elif(text == "casocontrario"):
    ...
    def parseCommand():
        ...
        if (Parser.tokens.actual.type in types):
            ...
            # Se for ( faz chamada de funcao
                ...
                raise ValueError(f"ERRO, esperava-se uma atribuicao com 'recebe' ou um abre parenteses (, mas recebeu {Parser.tokens.actual.type}")
            ...
            if Parser.tokens.actual.type == "OPENBR":
                ...
                if Parser.tokens.actual.type == "CLOSEBR":
                    ...
                    raise ValueError("ERRO: nao fechou parenteses do 'enquanto'")
            else:
                raise ValueError("ERRO: nao abriu parenteses do 'enquanto'")
            ...
            if Parser.tokens.actual.type == "OPENBR":
                ...
                if Parser.tokens.actual.type == "CLOSEBR":
                    ...
                    raise ValueError("ERRO: nao fechou parenteses do 'se_e_somente_se'")
            else:
                raise ValueError("ERRO: nao abriu parenteses do 'se_e_somente_se'")
    ...
    def parseFactor():
        ...
        if(Parser.tokens.actual.type == "INT"):
            ...
            if(Parser.tokens.actual.type == "OPENBR"):
                Parser.tokens.selectNext()
                if Parser.tokens.actual.type != "CLOSEBR":
                    raise Exception('ERRO: parenteses errado em um ler')
                Parser.tokens.selectNext()
            else:
                raise Exception('ERRO: parenteses errado em um ler')
...
def main():
    file = sys.argv[1]
    if(file.lower().endswith(('.porextenso'))):
        with open (file, 'r') as f:
            program = f.read()
    else:
        raise ValueError("ERRO: Passar arquivo com extensao .porextenso")
